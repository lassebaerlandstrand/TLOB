...
`run_experiments.py` is a convenience script that wraps `main.py` and supports three modes:

| Mode            | Description                                                        |
| --------------- | ------------------------------------------------------------------ |
| `all-horizons`  | Four sequential single-horizon runs (h=10,20,50,100). **Default.** |
| `single`        | One run for a specific horizon (`--horizon`).                      |
| `multi-horizon` | One joint run training all horizons simultaneously.                |

### All horizons — four separate runs
...
```sh
python run_experiments.py --mode single --model tlob --dataset btc --horizon 10
```
