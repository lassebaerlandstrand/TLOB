...
class Engine(LightningModule):
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        class_report = classification_report(
            targets, predictions, digits=4, output_dict=True
        )
    ...
    def on_test_epoch_end(self) -> None:
        ...
        predictions_path = os.path.join(
            cst.DIR_SAVED_MODEL, str(self.model_type), self.dir_ckpt, "predictions"
        )
    ...
    def model_checkpointing(self, loss):
        ...
        with self.ema.average_parameters():
            ...
            onnx_dir = os.path.join(
                cst.DIR_SAVED_MODEL, str(self.model_type), self.dir_ckpt, "onnx"
            )
            ...
            dummy_input = torch.randn(
                1, self.seq_size, self.num_features, device=self.device
            )
    ...
    def plot_pr_curves(self, recall, precision, is_wandb):
        ...
        plt.savefig(
            ...
            + str(self.model_type)
            + "/"
            + f"precision_recall_curve_{self.dataset_type}.svg"
        )
        # plt.show()
        plt.close()
...
def compute_most_attended(att_feature):
    ...
    att_feature = att_feature.transpose(
        1, 3, 0, 2, 4
    ...
    most_frequent_indices = np.zeros(
        (indices.shape[0], indices.shape[1], indices.shape[3]), dtype=int
    )
    ...
    for layer in range(indices.shape[0]):
        for head in range(indices.shape[1]):
            for seq in range(indices.shape[3]):
                ...
                avg_value = np.mean(
                    current_values[current_indices == most_frequent_index]
                )
