...
from models.original.mlplob import MLP
...
class TransformerLayer(nn.Module):
    def __init__(self, hidden_dim: int, num_heads: int, final_dim: int):
        ...
        self.attention = nn.MultiheadAttention(
            hidden_dim * num_heads, num_heads, batch_first=True, device=cst.DEVICE
        )
...
def sinusoidal_positional_embedding(
    ...
    )  # 10000^(2i/d_model), i is the index
...
def count_parameters(layer):
    print(
        f"Number of parameters: {sum(p.numel() for p in layer.parameters() if p.requires_grad)}"
    )
