...
def battery_load(path: str, all_features: bool, len_smooth: int, h: int, seq_size: int):
    ...
    The .npy format is auto-detected from the column count:
            54 cols → full format (10 msg + 40 LOB + 4 labels)
    ...
    all_features : bool
        If True, return LOB + message columns (50 features).
...
class BatteryDataBuilder:
    ...
    all_features : bool
        If True, synthesise and save 10 message columns (54-col format).
    ...
    def prepare_save_datasets(self):
        ...
        feature_cols = _NCOLS_FULL if self.all_features else _NCOLS_LOB
        feature_mode = "LOB + messages" if self.all_features else "LOB only"
    ...
    def _flush_day_buffer(self, day_buffer: list[dict], path: Path) -> None:
        ...
        save_kw: dict[str, np.ndarray] = dict(
            ...
            deliv_times=deliv_times,
            deliv_dates=deliv_dates,
        )
        if day_buffer[0].get("msg") is not None:
            save_kw["msgs"] = np.array([s["msg"] for s in day_buffer], dtype=np.float32)
    ...
    def _extract_all_snapshots(self, bin_path: Path, cache_hash: str) -> dict[str, np.ndarray]:
        ...
        dict with keys:
            ...
            lobs        : float32 (N, 40)
            msgs        : float32 (N, 10) — only present when all_features=True
        """
        import bitepy
                ...
                if lob_dict:
                    for dt_key in sorted(lob_dict.keys()):
                        ...
                        if self.all_features:
                            msg_row = self._synthesize_message_features(
                                lob_row,
                                prev_lob,
                                prev_ts,
                                current_time,
    ...
    def _synthesize_message_features(
        ...
        total_depth = max(sum_bid_vol + sum_ask_vol, 1e-8)
        book_imbalance = (sum_bid_vol - sum_ask_vol) / total_depth

        top_denom = max(vbuy1 + vsell1, 1e-8)
        top_imbalance = (vbuy1 - vsell1) / top_denom
        weighted_mid = (vbuy1 * sell1 + vsell1 * buy1) / top_denom
        weighted_mid_dev = weighted_mid - mid

        sum_all_volumes = float(np.sum(lob_row[1::2]))
        log_total_volume = math.log1p(max(sum_all_volumes, 0.0))
        ...
        return [
            float(log_time_delta),
            float(time_to_delivery_hrs),
            lifecycle_progress,
            direction,
            float(spread_bps),
    ...
    def _build_per_product_datasets(self, snapshots: dict[str, np.ndarray]):
        ...
        print(
            f"  Per-product split — train: {len(train_days)} delivery days, "
    ...
    def _normalise_features(self, features: np.ndarray, stats: dict | None) -> tuple[np.ndarray, dict]:
        ...
        if self.all_features:
            ...
            lob_df = pd.DataFrame(features[:, _N_MSG:])

            if stats is None:
                ms, mp, ss, sp = self._lob_stats(lob_df)
                lob_df, _, _, _, _ = z_score_orderbook(lob_df, ms, mp, ss, sp)
                msg_df, msg_mean, msg_std = self._normalise_messages_safe(msg_df)
                stats = dict(
                    lob_mean_size=ms,
                    lob_mean_price=mp,
                    lob_std_size=ss,
                ...
                lob_df, _, _, _, _ = z_score_orderbook(
                    lob_df,
                    stats["lob_mean_size"],
                    stats["lob_mean_price"],
                    stats["lob_std_size"],
                    stats["lob_std_price"],
                )
                msg_df, _, _ = self._normalise_messages_safe(
                    msg_df,
                    means=np.asarray(stats["msg_mean"], dtype=np.float64),
                    stds=np.asarray(stats["msg_std"], dtype=np.float64),
    ...
    def _normalise_messages_safe(
        ...
        """Simple per-column z-score normalisation for battery message features."""
        _EPS = 1e-8
        if columns is None:
            columns = list(data.columns)

        if means is None:
            means = data[columns].mean(axis=0).to_numpy(dtype=np.float64)
        else:
            means = np.asarray(means, dtype=np.float64)

        if stds is None:
            stds = data[columns].std(axis=0).to_numpy(dtype=np.float64)
        else:
        ...
        for i, col in enumerate(columns):
            df_out[col] = (df_out[col].astype(np.float64) - means[i]) / safe_stds[i]
    ...
    def _validate_lob_structure(self, lob: np.ndarray, name: str = ""):
        ...
        for lvl in range(self.n_lob_levels - 1):
            ...
            if bad > 0.05:
                print(
                    f"  WARNING {name}: sell prices not ascending at level {lvl + 1}→{lvl + 2}: {bad * 100:.1f}% rows"
                )
        ...
        for lvl in range(self.n_lob_levels - 1):
            ...
            if bad > 0.05:
                print(
                    f"  WARNING {name}: buy prices not descending at level {lvl + 1}→{lvl + 2}: {bad * 100:.1f}% rows"
                )
