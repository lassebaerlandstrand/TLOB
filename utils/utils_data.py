...
def z_score_orderbook(
    data, mean_size=None, mean_prices=None, std_size=None, std_prices=None
):
...
def compute_lob_diffs(
    data: torch.Tensor, n_lob: int = DEFAULT_N_LOB_FEATURES
) -> torch.Tensor:
...
def one_hot_encoding_type(data):
    ...
    one_hot_order_type = torch.nn.functional.one_hot(
        (data[:, 1]).to(torch.int64), num_classes=3
    ).to(torch.float32)
...
def tanh_encoding_type(data):
    data[:, 1] = torch.where(
        data[:, 1] == 1.0, 2.0, torch.where(data[:, 1] == 2.0, 1.0, data[:, 1])
    )
    data[:, 1] = data[:, 1] - 1
    return data
...
def labeling(X, len, h, label_mode: str = "absolute_change"):
    ...
    previous_ask_prices = np.lib.stride_tricks.sliding_window_view(
        X[:, 0], window_shape=len
    )[:-h]
    previous_bid_prices = np.lib.stride_tricks.sliding_window_view(
        X[:, 2], window_shape=len
    )[:-h]
    future_ask_prices = np.lib.stride_tricks.sliding_window_view(
        X[:, 0], window_shape=len
    )[h:]
    future_bid_prices = np.lib.stride_tricks.sliding_window_view(
        X[:, 2], window_shape=len
    )[h:]
    ...
    elif label_mode == "percent_change":
        # Original definition used in older versions / paper-style setups.
        eps = 1e-12
        price_change = (future_mid_prices - previous_mid_prices) / np.where(
            np.abs(previous_mid_prices) > eps,
            previous_mid_prices,
            np.sign(previous_mid_prices) * eps + (previous_mid_prices == 0) * eps,
        )
    else:
        raise ValueError(
            f"Unknown label_mode='{label_mode}'. Expected 'absolute_change' or 'percent_change'."
        )
